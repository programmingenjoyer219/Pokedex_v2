...
export default function SearchBox({ getPokemonsByType, getPokemonsByName }) {
    ...
    return (
        <div className='flex flex-col items-center space-y-4 w-full'>
            <form onSubmit={handleSubmit} className='flex flex-col space-y-4 sm:flex-row items-center rounded-lg justify-between space-x-2 w-full p-6 bg-white shadow-lg border border-gray-200'>
                <MagnifyingGlassIcon className='text-blue-600 h-6 w-6' />
                <input
                    value={name}
                    onChange={(e) => setName(e.target.value)}
                    className='flex-1 p-3 text-center sm:text-left rounded-md border border-gray-300 focus:outline-none focus:border-blue-500 focus:ring focus:ring-blue-200 placeholder:text-blue-300 text-gray-800 font-semibold'
                    type="text"
            ...
            <Marquee className='w-full p-4 border-2 border-blue-500 rounded-md bg-white shadow-lg' pauseOnHover>
                {
